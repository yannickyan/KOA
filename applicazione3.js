...
root.get("/login.html", async (ctx,next) =>{
  ...
  {  
    ...
    ctx.body="successo del caricamento del file"

  }).get("/risultati.html",async (ctx,next)=>{
     if(ctx.query.bol=="true")
      {
      const date=Date.now();
      ...
      var oggetto={partie:render.toString(),
      bol:true};
      var jsonsend=JSON.stringify(oggetto)
      ctx.body=jsonsend;
...
// dedicato un middleware per creazione dell'array di stringhe con le partite che rispettano il tempo
    app.use(async (ctx, next) => {
      const accesso=ctx.session.tempoaccesso;
      const risultato=[];
      ...
      while ((line = await read.next()) !== false) {
        ...
        const Hourmin=tempi.split(".");
        var d=new Date().setHours(Hourmin[0],Hourmin[1]);
        if(accesso >= d)
        {
